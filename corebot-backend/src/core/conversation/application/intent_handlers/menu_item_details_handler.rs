...
pub struct MenuItemDetailsIntentHandler<D: DomainGatewayPort> {
    domain_gateway: Arc<D>,
}

impl<D: DomainGatewayPort> MenuItemDetailsIntentHandler<D> {
    pub fn new(domain_gateway: Arc<D>) -> Self { Self { domain_gateway } }
}

impl<D: DomainGatewayPort + Send + Sync> IntentHandler for MenuItemDetailsIntentHandler<D> {
    fn intent(&self) -> IntentId { IntentId::AskMenuItemDetails }
    ...
    fn handle(&self, input: IntentHandlerInput<'_>) -> StateHandlerResult {
        ...
        let raw = self.domain_gateway.get_menu_item_details(menu_item, allergen);
...
fn parse_item_details_reply(raw: &str, lang: &str) -> String {
    if let Some(payload) = raw.strip_prefix("contains:") {
        ...
        return t!("intent.ask_menu_item_details.contains.reply", locale = lang, item = item, allergen = allergen).to_string();
    }
    if let Some(payload) = raw.strip_prefix("not_contains:") {
        let mut parts = payload.splitn(2, '|');
        let item = parts.next().unwrap_or("");
        let allergen = parts.next().unwrap_or("");
        return t!("intent.ask_menu_item_details.not_contains.reply", locale = lang, item = item, allergen = allergen).to_string();
    }
    if let Some(payload) = raw.strip_prefix("item_details:") {
        ...
        return t!("intent.ask_menu_item_details.full.reply", locale = lang, item = name, price = price, dietary = dietary, allergens = allergens).to_string();
    }
    if let Some(payload) = raw.strip_prefix("allergen_found:") {
        let mut parts = payload.splitn(2, '|');
        let allergen = parts.next().unwrap_or("");
        let items = parts.next().unwrap_or("");
        return t!("intent.ask_menu_item_details.allergen_found.reply", locale = lang, allergen = allergen, items = items).to_string();
    }
    if raw.starts_with("no_allergen_match:") || raw.starts_with("item_unknown:") {
        return t!("intent.ask_menu_item_details.not_found.reply", locale = lang).to_string();
    }
