...
pub struct AskAccessibilityIntentHandler<'a, B> {
    business_info_repository: &'a B,
}

impl<'a, B> AskAccessibilityIntentHandler<'a, B> {
...
impl<B> IntentHandler for AskAccessibilityIntentHandler<'_, B>
where
    B: RestaurantBusinessInfoRepositoryPort + Send + Sync,
{
    ...
    async fn handle(&self, input: IntentHandlerInput<'_>) -> StateHandlerResult {
            ...
            .await
        {
            Ok(facts) => facts
                .iter()
                .find(|fact| fact.fact_type == "accessibility")
                .map(|fact| {
                    t!(
                ...
                .unwrap_or_else(|| t!("intent.ask_accessibility.reply", locale = lang).to_string()),
            Err(_) => t!("intent.ask_accessibility.reply", locale = lang).to_string(),
        };
        StateHandlerResult {
