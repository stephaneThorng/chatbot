...
class IntentClassifier:
    ...
    def classify(
        ...
        regex_result = None
        available_intents = self._available_intents(domain)
        context_result = self.context_resolver.resolve_intent(text=text, context=context, available_intents=available_intents)
        if context_result is not None:
            return IntentResult(
                name=context_result.name,
                confidence=context_result.confidence,
                fast_path=context_result.fast_path,
                source=context_result.source,
                alternatives=context_result.alternatives,
