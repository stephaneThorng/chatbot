...
import dev.stephyu.core.chat.application.intent.IntentCategory
...
class IntentResolver(
    ...
    fun resolve(
        ...
        if (!session.hasCurrentWorkflow() && shouldIgnoreIdleMessage(normalized)) {
            return IntentRoutingDecision.Unknown
        }
        ...
        if (isTopicContinuation(normalized)) {
            ...
            if (topicIntent != null && supportsTopicContinuation(topicIntent)) {
                return IntentRoutingDecision.Accept(topicIntent, source = "topic_memory")
            }
            return IntentRoutingDecision.Unknown
        }
        ...
        if (shouldClarify(primary.name, primary.confidence, secondary, analysis)) {
            return IntentRoutingDecision.Clarify(
                primary = primary.name,
                secondary = secondary!!.first,
            )
        }
        return IntentRoutingDecision.Accept(primary.name, source = "nlp")
    }
    ...
    private fun resolveForActiveWorkflow(
        ...
        val workflowIntent = session.currentWorkflow?.ownerIntent ?: return IntentRoutingDecision.Unknown
        if (isWorkflowScopedMessage(normalized)) {
            return IntentRoutingDecision.Accept(workflowIntent, source = "workflow_scope")
        }
        if (allowsDuringWorkflow(analysis.intent.name) && analysis.intent.confidence >= confidenceThreshold) {
            return IntentRoutingDecision.Accept(analysis.intent.name, source = "nlp")
        }
        return IntentRoutingDecision.Accept(workflowIntent, source = "workflow_owner")
    }
    ...
    private fun resolvePendingDisambiguation(
        ...
        explicitDisambiguationChoice(normalized, pending.candidates)?.let { chosen ->
            return IntentRoutingDecision.Accept(chosen, source = "pending_disambiguation")
        }
    ...
    private fun hasEntitySupport(intent: IntentName, analysis: NlpAnalysis): Boolean {
        val supported = conversationConfig.findIntentPolicy(intent).entitySupport
    ...
    private fun explicitDisambiguationChoice(
        ...
        candidates.firstOrNull { candidate ->
            conversationConfig.findIntentPolicy(candidate).disambiguationLabels.any { label ->
                Regex("""\b${Regex.escape(label)}\b""").containsMatchIn(message)
            }
        }

    private fun isClarifiable(intent: IntentName): Boolean =
        conversationConfig.findIntentPolicy(intent).clarifiable

    private fun isInformational(intent: IntentName): Boolean =
        conversationConfig.findIntentPolicy(intent).category == IntentCategory.INFORMATIONAL

    private fun supportsTopicContinuation(intent: IntentName): Boolean =
        conversationConfig.findIntentPolicy(intent).supportsTopicContinuation

    private fun allowsDuringWorkflow(intent: IntentName): Boolean =
        conversationConfig.findIntentPolicy(intent).allowDuringWorkflow
